...
export function ThemeCard({ theme, name, description, icon: Icon, selected, onSelect }: ThemeCardProps) {
  return (
    <button
      ...
      className={[
        'relative flex flex-col gap-4 p-4 rounded-2xl border text-left min-w-0',
    ...
    >
      ...
      <span className="w-10 h-10 bg-[#0d0d1e] border border-[#252545] rounded-xl flex items-center justify-center text-violet-400 shrink-0">
        <Icon size={19} strokeWidth={1.5} />
      </span>
      <div>
        <p className="text-white font-bold text-[15px] leading-tight">{name}</p>
        <p className="text-gray-400 text-sm mt-1.5 leading-snug">{description}</p>
      </div>
    </button>
  )
}
